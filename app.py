...
# --- SIDEBAR NAVIGATION & CONTEXT-AWARE SEARCH ---
...
# Context-Aware Sidebar Search Bar
global_search_code = ""
area_search_query = ""

if st.session_state["selected_area"] is None:
    st.sidebar.markdown("### 🔍 Global Search (All Areas)")
    global_search_code = st.sidebar.text_input("Exact Material Code:", "").strip()
else:
    st.sidebar.markdown(f"### 🔍 Filter {st.session_state['selected_area']}")
    area_search_query = st.sidebar.text_input("Search Name / Mat Code:", "").strip()
...
# --- GLOBAL EXACT MATERIAL CODE SEARCH MODE (When on Home Grid and user types in sidebar) ---
if st.session_state["selected_area"] is None and global_search_code:
    st.markdown(f"""
        <div style="background: linear-gradient(135deg, #ffffff 0%, #f1f5f9 100%); padding: 30px; border-radius: 16px; border: 1px solid #cbd5e1; box-shadow: 0 10px 25px rgba(0,0,0,0.03); text-align: center; margin-bottom: 25px;">
            <h1 style="color: #0f172a !important; margin: 0; font-size: 28px; font-weight: 800;">🔢 Global Search Results for: {global_search_code}</h1>
            <p style="color: #475569 !important; margin-top: 8px; font-size: 14px;">Scanning across all connected plant areas...</p>
        </div>
    ...
    for area_key, area_cfg in AREA_CONFIGS.items():
        ...
        try:
            ...
            if mat_col in df_area.columns:
                ...
                mask = cleaned_codes == global_search_code
    ...
    if all_results:
        ...
        st.success(f"Found match for material code **{global_search_code}** in {len(res_df)} location(s) across the plant!")
        ...
        st.info(f"No item with exact material code '{global_search_code}' found across the connected plant areas.")
...
# --- HOD LANDING PAGE (Dynamic 3-Column Block Grid for all 8 areas) ---
    ...
    st.markdown("""
        <div style="background: linear-gradient(135deg, #ffffff 0%, #f1f5f9 100%); padding: 35px; border-radius: 16px; border: 1px solid #cbd5e1; box-shadow: 0 10px 25px rgba(0,0,0,0.03); text-align: center; margin-bottom: 35px;">
            ...
            <p style="color: #475569 !important; margin-top: 10px; font-size: 15px; font-weight: 500;">Select an operational area block below or use sidebar search for global material lookup</p>
        </div>
    """, unsafe_allow_html=True)
...
# --- ACTIVE AREA DASHBOARD VIEW (With area-specific sidebar filtering) ---
else:
    ...
    try:
        ...
        # Apply area-specific search filter if typed in the sidebar
        if area_search_query:
            q_low = area_search_query.lower()
            mask_name = df[NAME_COL].str.lower().str.contains(q_low, na=False)
            mask_mat = df[MAT_COL].apply(lambda x: q_low in clean_material_code(x).lower()) if MAT_COL in df.columns else False
            df = df[mask_name | mask_mat]
            st.info(f"Filtered by search query: '{area_search_query}' in {current_area}")

        unique_names_ordered = df[NAME_COL].unique()

        if len(unique_names_ordered) == 0:
            st.warning(f"No matching items found for '{area_search_query}' in {current_area}.")
